#!/bin/bash
# This is a test script for the round-robin strategy.
...
cd ${SCRIPT_DIR}
./load_balancer -c test-round-robin-standard.json > /dev/null 2>&1 &
...
# requests are distributed between the three servers.
cd ${SCRIPT_DIR}
for i in {1..3}
do
    curl -s localhost:5000 > curl.out
    if grep -q "This file is being served from server ${i}." curl.out; then
