...
void sigpipe_handler(int signum) { return; }
...
int HealthReporter::get_port() { return this->port; }
...
void HealthReporter::start() {
  ...
  while (true) {
    ...
    HealthReporterConnection *conn =
        new HealthReporterConnection(client_sock, this->enc_key);
...
bool HealthReporterConnection::handle_lb_handshake() {
  ...
  string iv_str =
      string(HR_HANDSHAKE_MSG_4) + string((char *)this->iv, 16) + string("\n");
...
HealthReport_t HealthReporterConnection::generate_health_report() {
  ...
  if (meminfo == NULL) {
    ...
    if (mem_total == -1 || mem_available == -1) {
      ...
      report.mem_load = (int)(((float)mem_total - (float)mem_available) /
                              (float)mem_total * 100);
    }
  }
...
EncryptedHealthReport *
HealthReporterConnection::encrypt_health_report(HealthReport_t report) {
  ...
  const std::vector<unsigned char> key =
      plusaes::key_from_string(&key_ptr); // 16-char = 128-bit
  ...
  plusaes::encrypt_cbc((unsigned char *)raw_data.data(), raw_data.size(),
                       &key[0], key.size(), &this->iv, &encrypted[0],
                       encrypted.size(), true);
...
void HealthReporterConnection::handle_connected() {
  ...
  while (true) {
    ...
    EncryptedHealthReport_t *encrypted_health_report_struct =
        encrypt_health_report(report);
    std::vector<unsigned char> encrypted_health_report =
        encrypted_health_report_struct->encrypted_health_report;
    ...
    int size = Base64encode(base64_encoded_buffer,
                            (char *)encrypted_health_report.data(),
                            encrypted_health_report.size());

    // Convert to string
    std::string encrypted_health_report_str =
        std::string(base64_encoded_buffer);

    // Append newline and size of encrypted health report
    encrypted_health_report_str += "\n";
    encrypted_health_report_str +=
        std::to_string(encrypted_health_report.size());

    // Send encrypted health report
    if (write(this->socket, encrypted_health_report_str.c_str(),
              encrypted_health_report_str.size()) < 0) {
