...
//
// Library URL: https://github.com/kkAyataka/plusaes
//
...
inline void mix_columns(State &state) {
  for (int i = 0; i < kBlockSize; ++i) {
    ...
    state[i] =
        (v0_2 ^ v1_3 ^ v2_1 ^ v3_1) << 0 | (v0_1 ^ v1_2 ^ v2_3 ^ v3_1) << 8 |
...
template <typename T> unsigned long ceil(const T v) {
...
template <std::size_t N> std::bitset<N> inc32(const std::bitset<N> X) {
 ...
 * @param [in]  key key bytes. The key length must be 16 (128-bit), 24 (192-bit)
 * or 32 (256-bit).
 ...
 * @param [out] padded_size If this value is NULL, this function does not remove
 * padding. If this value is not NULL, this function removes padding by PKCS and
 * returns padded size using padded_size.
 * @since 1.0.0
 */
 ...
 */
inline Error
decrypt_cbc(const unsigned char *data, const unsigned long data_size,
            const unsigned char *key, const unsigned long key_size,
 ...
 * But usually you should use the other overloaded function whose iv and tag
 * size is fixed.
 *
 * @returns kErrorOk
...
inline Error encrypt_gcm(unsigned char *data, const std::size_t data_size,
  ...
  detail::gcm::calc_gcm_tag(data, data_size, aadata, aadata_size, key, key_size,
 ...
 * @param [in] key_size Cipher key size. This value must be 16 (128-bit), 24
 * (192-bit), or 32 (256-bit).
