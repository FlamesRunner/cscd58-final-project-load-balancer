...
void LoadBalancer::listener()
{
    ...
    // Bind listener
    errno = 0;
    bind(listener_socket, (struct sockaddr *)&listener_address, sizeof(listener_address));
    if (errno != 0)
    {
    ...
    while (true)
    {
        ...
        int connection_socket = accept(listener_socket, (struct sockaddr *)&client_address, &client_address_size);
        ...
        connect(node_socket, (struct sockaddr *)&node_address, sizeof(node_address));

        // Forward traffic from client to node
        thread forward_client_to_node_thread(&LoadBalancer::forward_traffic, this, connection_socket, node_socket);
        forward_client_to_node_thread.detach();

        // Forward traffic from node to client
        thread forward_node_to_client_thread(&LoadBalancer::forward_traffic, this, node_socket, connection_socket);
