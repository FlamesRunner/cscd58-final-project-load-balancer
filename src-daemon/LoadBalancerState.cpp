    ...
    while (true)
    {
        ...
        while (true)
        {
            ...
            if (payload_len <= 0)
            {
                ...
                cout << "Invalid payload length received (" << payload_len << "). Ignoring." << endl;
                node.set_status(NODE_STATUS_DOWN);
                continue;
            }
            ...
            plusaes::decrypt_cbc((unsigned char *)decoded_payload, payload_len, (unsigned char *)this->config.enc_key.c_str(), this->config.enc_key.size(), &iv, &decrypted_payload[0], decrypted_payload.size(), &padded_size);

            // Deserialize the payload
            HealthReport_t hr = HealthReportSerialzer::deserialize(string(decrypted_payload.begin(), decrypted_payload.end()));
...
void LoadBalancerState::invalidator_thread(vector<shared_ptr<NodeState>> nodes)
{
    while (true)
    {
        ...
        for (shared_ptr<NodeState> node_ptr : nodes)
        {
            ...
            {
                if (time(NULL) - node.get_last_checked() > this->get_config().time_until_node_down)
                {
                    cout << "No report received from node " << node.node_config.name << " in the last 5 seconds. Marking as down." << endl;
                    node.set_status(NODE_STATUS_DOWN);
                }
            }
        }
...
void LoadBalancerState::start_rt_checks(void)
{
    ...
    {
        cerr << "Starting real-time health check for node " << node->node_config.name << "..." << endl;
        thread rt_check_thread(&LoadBalancerState::lb_rt_thread, this, node);
        rt_check_thread.detach();
    }

    // Start invalidation thread
    thread invalidator_thread(&LoadBalancerState::invalidator_thread, this, this->getNodes());
