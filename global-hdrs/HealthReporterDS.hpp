...
#define HR_HANDSHAKE_MSG_4                                                     \
  "4 IV " // IV is appended to this message along with a newline

enum HealthReporterState { HR_STATE_LB_HANDSHAKE, HR_STATE_CONNECTED };
...
public:
  static std::string serialize(HealthReport_t report) {
    unsigned char *encrypted_health_report =
        new unsigned char[sizeof(HealthReport_t)];
    memcpy(encrypted_health_report, &report, sizeof(HealthReport_t));
    std::string raw_data =
        std::string((char *)encrypted_health_report, sizeof(HealthReport_t));
    return raw_data;
  };
